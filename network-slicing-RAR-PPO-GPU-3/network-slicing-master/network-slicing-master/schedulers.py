...
class ProportionalFair:
    ...
    def allocate(self, ues, n_prb, error_bound = 0.1):
        ...
        n_ues = len(ues)
        ue_rbs = np.zeros(n_ues, dtype = np.int64)
        ue_mcs = np.zeros(n_ues, dtype = np.int64)
        ue_queue = np.zeros(n_ues, dtype = np.int64)
        ue_rate = np.zeros(n_ues, dtype = np.int64)
        ue_bits = np.zeros(n_ues, dtype = np.int64)
        ...
        for i, ue in enumerate(ues):
            ...
            if prbs:
                ...
                #########################################################################################################
                # if isinstance(snr_values, np.ndarray) and snr_values.size == 0:
                #     print("EMPTY snr_values:", "prb_i", prb_i, "prbs", prbs, "ue.snr.shape", np.asarray(ue.snr).shape)
                ###########################################################################################################
