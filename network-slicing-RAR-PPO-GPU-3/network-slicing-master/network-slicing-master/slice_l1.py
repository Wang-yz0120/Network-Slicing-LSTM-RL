...
class SliceL1mMTC:
    ...
    def slot(self):
        ...
        # --- 强制用 int，避免后续切片 [:n_tx] 报类型错 ---
        n_carriers = int(self.n_prbs)  # in NB-IoT one carrier is 1 PRB
        n_tx = int(min(n_carriers, int(self.n_users)))
        ...
        for slice_ran in self.slices_ran:
            ...
            if users_in_slice > 0:
                ...
                avg_rep = int(np.rint(self.repetitions[slice_indexes].mean()))
...
class SliceL1eMBB:
    ...
    def reset(self):
        ...
        for slice_ran in self.slices_ran:
            slice_ran.reset() 
         # >>> 新增：同步清空 SNR 用户表（如有 reset 就调 reset） <<<
        if hasattr(self.snr_generator, "reset") and callable(self.snr_generator.reset):
            self.snr_generator.reset()
        elif hasattr(self.snr_generator, "users"):
            try:
                self.snr_generator.users.clear()
            except Exception:
                pass   
    ...
    def extract_users(self, ue_id_list):
        for ue_id in ue_id_list:
            # 1) 本地 UE 列表移除
            if self.ues:
                self.ues = [ue for ue in self.ues if getattr(ue, "id", None) != ue_id]
            # 2) SNR 用户表安全删除（不存在就忽略）
            if hasattr(self.snr_generator, "extract_user") and callable(self.snr_generator.extract_user):
                try:
                    self.snr_generator.extract_user(ue_id)
                except KeyError:
                    pass
            elif hasattr(self.snr_generator, "users"):
                self.snr_generator.users.pop(ue_id, None)
    ...
    def slot(self):
        ...
        for ue in self.ues:
            ...
            if self.n_prbs > 0:
                ...
                ###############################################
                snr_arr = np.asarray(snr).reshape(-1)
                # print("DEBUG snr_generator =", type(self.snr_generator).__name__,
                #       "len(snr)=", snr_arr.size,
                #       "expected n_prbs=", self.n_prbs)
                snr_slice = np.asarray(snr[self.prb_slice]).reshape(-1)

                if snr_slice.size != self.n_prbs:
                    print("MISMATCH:",
                          "slice_n_prbs=", self.n_prbs,
                          "snr_slice.size=", snr_slice.size,
                          "prb_slice=", self.prb_slice,
                          "full_snr_len=", np.asarray(snr).size,
                          "ue_id=", ue.id)
                ###############################################
                try:
        ...
        if queued_data > 0 and self.n_prbs > 0:
            ...
            for ue in self.ues:
                ...
                #print(f"ue prbs: {ue.prbs}, ue p: {ue.p}")
