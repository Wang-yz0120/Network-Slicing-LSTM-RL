...
class ExperimentConfig:
    scenario_id: int = 1
    exp_tag: str = "default"
    algo_name: str = "LSTMDDPG"
    use_prediction: bool = True
    use_cross_step: bool = True
    use_dense_reward: bool = True
    dense_reward_kwargs: Dict[str, Any] = field(default_factory=lambda: dict(
        barrier=10.0, gamma=3.0, kappa=3.0, alpha=1.0, beta=0.0, clip_abs=1.0
    ))
    online_finetune: bool = False
    delay_steps: int = 1
    debug: bool = False
    seed: int = 0
    train_steps_a: int = TRAIN_STEPS_A
    train_steps_b: int = TRAIN_STEPS_B
    train_steps_c: int = TRAIN_STEPS_C
    eval_steps: int = EVALUATION_STEPS
    n_envs: int = N_ENVS
    control_steps: int = CONTROL_STEPS
    penalty: int = PENALTY
    pretrained_path: str = "./traffic_lstm_pretrained_3N.pth"
    result_root: str = "./results/ablations"
    model_root: str = "./trained_models/ablations"

    def total_train_steps(self) -> int:
        return int(self.train_steps_a + self.train_steps_b + self.train_steps_c)

    def result_dir(self) -> str:
        return os.path.join(self.result_root, f"scenario_{self.scenario_id}", self.exp_tag)

    def model_dir(self) -> str:
        return os.path.join(self.model_root, f"scenario_{self.scenario_id}", self.exp_tag)
...
def _force_save(vec_env, run_idx):
    env_list = vec_env.envs if isinstance(vec_env, DummyVecEnv) else [vec_env]

    for i, env in enumerate(env_list):
        chain = _unwrap_chain(env)
        report = _find_wrapper(env, ReportWrapper)
        if report is None:
            print("[FORCE SAVE] No save_results() found. Chain: " + " -> ".join(type(x).__name__ for x in chain))
            continue
        try:
            report.save_results()
            print(f"[FORCE SAVE] results saved via {type(report).__name__} for run {run_idx} env {i} | chain: " +
                  " -> ".join(type(x).__name__ for x in chain))
        except Exception as e:
            print(f"[FORCE SAVE] save_results() on {type(report).__name__} raised: {e}")
...
def _make_wrapped_vec_env(cfg: ExperimentConfig, run_idx: int, total_steps: int,
                          save_dir: str):
    def _make_single_env_thunk(worker_idx: int):
        def _thunk():
            rng = np.random.default_rng(seed=cfg.seed * 1000 + run_idx * 100 + worker_idx)
            env = create_env(rng, cfg.scenario_id, penalty=cfg.penalty)

            nb = _get_node_b_from_env(env)
            n_prbs = getattr(nb, "n_prbs", None)
            n_prbs = int(n_prbs) if n_prbs is not None else None
            if nb is not None:
                setattr(nb, "enable_cross_step", bool(cfg.use_cross_step))
                setattr(nb, "debug", bool(cfg.debug))

            if cfg.use_dense_reward:
                env = SlaDenseRewardWrapper(env, **cfg.dense_reward_kwargs)
            if cfg.use_prediction:
                env = LSTMPredictWrapper(
                    ...
                    pretrained_path=cfg.pretrained_path,  # 关键：加载离线预训练权重
                    online_finetune=cfg.online_finetune,
                    ...
                    batch_size=64,
                    grad_clip=1.0,
                    autosave_path=os.path.join(cfg.model_dir(), f"lstm_finetune_run{run_idx}.pt"),
                    autosave_every_updates=5000,
                    debug=cfg.debug,
                )

            total_with_eval = int(total_steps + cfg.eval_steps)
            # save per-env results with run+env identifiers
            env_id = f"run{run_idx}_env{worker_idx}"
            env = ReportWrapper(
                env,
                steps=total_with_eval,
                control_steps=cfg.control_steps,
                env_id=env_id,
                path=save_dir,
                verbose=VERBOSE,
                n_prbs=n_prbs,
                use_cross_step=cfg.use_cross_step,
                debug=cfg.debug,
            )
            ...
            env = DelayActionWrapper(env, delay_steps=cfg.delay_steps, debug=cfg.debug)
            return env
        return _thunk

    thunks = [_make_single_env_thunk(i) for i in range(cfg.n_envs)]
...
class RLEvaluator():
    def __init__(self, cfg: ExperimentConfig):
        self.cfg = cfg
        self.scenario = cfg.scenario_id
        self.algo_name = cfg.algo_name
        self.res_path   = cfg.result_dir()
    ...
    def evaluate(self, run_idx: int):
        print(f'start evaluation of scenario {self.scenario} run {run_idx} algorithm {self.algo_name} tag {self.cfg.exp_tag}')
        set_random_seed(self.cfg.seed + run_idx)
        _save_json(os.path.join(self.res_path, f"config_run{run_idx}.json"), asdict(self.cfg))

        total_steps = self.cfg.total_train_steps()
        vec_env, outer_env0 = _make_wrapped_vec_env(self.cfg, run_idx, total_steps, self.res_path)
        ...
        if self.cfg.train_steps_a > 0:
            model.learn(
                total_timesteps=self.cfg.train_steps_a,
                progress_bar=True,
                reset_num_timesteps=True
            )

        _set_stage_params(vec_env, "B")
        if self.cfg.train_steps_b > 0:
            model.learn(
                total_timesteps=self.cfg.train_steps_b,
                progress_bar=True,
                reset_num_timesteps=False  # 连续时间轴
            )

        if self.cfg.train_steps_c > 0:
            _set_stage_params(vec_env, "C")
            model.learn(
                total_timesteps=self.cfg.train_steps_c,
                progress_bar=True,
                reset_num_timesteps=False  # 连续时间轴
            )

        print('trainning done!')
        save_path = os.path.join(self.model_path, f'{self.algo_name}_{self.cfg.exp_tag}_agent_{run_idx}')
        model.save(save_path + ".zip")
        print('model saved')

        ###################################################################################
        if self.algo_name in {"RaRPPO", "LSTMDDPG"} and self.cfg.use_prediction:
            # 训练完成后，调用 lstm_wrapper.save_predictions() 保存预测值
            lstm_env = _find_lstm_wrapper(outer_env0)
            if lstm_env is not None:
                lstm_env.save_predictions(path=os.path.join(self.res_path, f"lstm_predictions_run{run_idx}.npz"))
...
def run_experiment(cfg: ExperimentConfig, run_idx: int = 0):
    evaluator = RLEvaluator(cfg)
    evaluator.evaluate(run_idx)
# ================================== main ===================================
if __name__ == '__main__':
    ...
    for scenario, alg_name in product(scenarios, algorithms.keys()):
        cfg = ExperimentConfig(scenario_id=scenario, algo_name=alg_name)
        evaluator = RLEvaluator(cfg)
        for run in run_list:
