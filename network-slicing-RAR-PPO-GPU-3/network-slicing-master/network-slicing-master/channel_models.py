    ...
    def __init__(self, rng, model_name, n_prbs = 100, filenames = FILESNAMES, user_ids = None):
        ...
        for filename in filenames:
            ...
            if n_prbs > 100:
                ...
                #########################################
                sample_matrix = df.to_numpy()
                rows = sample_matrix.shape[0]

                # 强制让 fading 的“频域长度”与 n_prbs 一致
                if rows >= n_prbs:
                    sample_matrix = sample_matrix[:n_prbs, :]
                else:
                    reps = (n_prbs + rows - 1) // rows
                    sample_matrix = np.vstack([sample_matrix] * reps)[:n_prbs, :]

                self.samples.append(sample_matrix)
                print("[SINRSelectiveFading] sample_matrix.shape =", sample_matrix.shape)
