...
class NodeB():
    ...
    def release_expired_prb(self):
        ...
        释放已过期的跨step PRB资源。
        """
        if not self.enable_cross_step:
            self.slice_leases.clear()
            self.remaining_prb = int(self.n_prbs)
            return
        # 遍历租约，释放过期的PRB
