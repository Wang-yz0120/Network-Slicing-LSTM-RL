...
class SlaDenseRewardWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        if self.slice_meta:
            last_meta = self.slice_meta[max(self.slice_meta.keys())]
            last_meta["obsT"] = last_meta["slots_per_step"] * self.slots_length
            if DEBUG:
                print("[SlaDense] init slice_meta:")
                for gid, m in self.slice_meta.items():
                    print(f"  gid={gid}: type={m['type']}, SLA_keys={list(m['SLA'].keys())}")
                print(f"SLA DENSE slots_per_step:{last_meta['slots_per_step']},obT:{last_meta['obsT']}")
